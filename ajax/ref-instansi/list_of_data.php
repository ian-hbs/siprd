		...
		<tbody>";
			...
			while($row = $list_of_data->FetchRow())
			{
				...
				<td align='center'>";
					...
					if($editAccess)
		                echo "<a href='ajax/".$fn."/form_content.php?act=edit&id=".$kd_instansi."&fn=".$fn."&men_id=".$men_id."' title='Edit' class='btn btn-xs btn-default' id='edit_".$no."' data-toggle='modal' data-target='#remoteModal'>";
		            else
	                ...
	                echo "
	                <input type='hidden' id='ajax-req-dt' name='id' value='".$kd_instansi."'/>
